...
num_previous_orders = np.clip(
    (customer_tenure_days / rng.uniform(2, 180, N)).round(0),
    0,
    None,
)
...
num_previous_returns = np.minimum(
    rng.binomial(num_previous_orders.astype(int), base_return_rate),
    num_previous_orders,
)

delivery_distance_km = np.clip(
    rng.gamma(3, 90, N), 2, 2200
).round(1)

delivery_days = np.clip(
    rng.normal(4.5, 2.2, N), 1, 21
).round(0)

is_weekend_order = rng.integers(0, 2, N)

rating_given = rng.integers(1, 6, N).astype(float)
missing_mask = rng.random(N) < np.where(
    payment_method == "COD", 0.22, 0.06
)
...
z = (
    ...
    + 0.10 * (delivery_days - 4.5)
    + 2.30 * (price_inr / base_price["Electronics"][1])
...
print(
    "Rows:", len(df),
    "| Return rate:", round(df["returned"].mean(), 4)
)